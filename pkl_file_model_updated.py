...
class ResE4D1(nn.Module):
    def __init__(self, obs_shape1: tuple, obs_shape2: tuple, obs_shape3: tuple, obs_shape4: tuple, z_dim1: int, z_dim2: int, z_dim3: int, z_dim4: int, n_samples: int=4, n_res_blocks: int=3):
        ...
        self.model_name = f"SpectralResE4D1_{int(z_dim1 + z_dim2 + z_dim3 + z_dim4)}"
    ...
    def forward(self, obs1, obs2, obs3, obs4, random_bottle_neck=False):
        ...
        z_sample = z_sample / torch.norm(z_sample, p=2)
        nuc_loss = torch.norm(z_sample, p='nuc', dim=(0, 1)) / batch_size
        total_spec_snr = -10 * torch.log10(torch.mean((obs - obs_dec) ** 2) / torch.mean(obs ** 2))
        ...
        return obs_dec, torch.mean(mse), nuc_loss, 0, cos_loss, spec_loss_dict, total_spec_snr, psnr
...
class ResE2D1(nn.Module):
    def __init__(self, obs_shape1: tuple, obs_shape2: tuple, z_dim1: int, z_dim2: int, n_samples: int=4, n_res_blocks: int=3):
        ...
        self.model_name = f"SpectralResE2D1_{int(z_dim1 + z_dim2)}"
    def get_model_name(self):
    ...
    def forward(self, obs1, obs2, random_bottle_neck=False):
        ...
        # print("sample size: ",z_sample.shape)
        # print("Alarming1: ",z_sample)
        ### Normalize
        # if z_sample.shape[0] > 1:
        z_sample = z_sample - z_sample.mean(dim=0)

        ### nuclear loss 
        # print("Alarmin2: ",z_sample,)
        # print("Alarming_3: ",torch.norm(z_sample, p=2))
        z_sample = z_sample / torch.norm(z_sample, p=2)
        nuc_loss = torch.norm(z_sample, p='nuc', dim=(0, 1)) / batch_size
        total_spec_snr = -10 * torch.log10(torch.mean((obs - obs_dec) ** 2) / torch.mean(obs ** 2))
...
class ResE1D1(nn.Module):
    def __init__(self, obs_shape: tuple, z_dim: int, n_samples: int=4, n_res_blocks: int=3): # noise=0.01):
        ...
        self.model_name = f"SpectralResE1D1_{int(z_dim)}"
    ...
    def forward(self, obs):
        ...
        ### weight parameters recommended by VIC paper: 25, 25, and 10
        # return obs_dec, torch.mean(mse), nuc_loss, 0, 0, psnr
        return obs_dec, torch.mean(mse), nuc_loss, 0, 0, spec_loss_dict, total_spec_snr, psnr
