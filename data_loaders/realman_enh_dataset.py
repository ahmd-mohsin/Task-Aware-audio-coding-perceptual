...
def load_wav(
    ...
    for chn_idx in channels:
        ...
        wav_chn, sr = torchaudio.load(wav_chn_path, frame_offset=start, num_frames=end-start, backend='soundfile') #[C,T]
        wav_chn = wav_chn[0,:]
...
class RealisticAudioDataset(Dataset):
    ...
    def __getitem__(self, index_seed: tuple[int, int]):
        ...
        # print(str(self.target_dir / str(self.uttrs[idx]).removeprefix(str(self.record_dir) + '/').replace('_CH0', '')))
        ...
        if self.dataset != 'predict':  # for predict, the original level should given
            ...
            target[:] *= scale_value
        # Extract components
        save_to_file = str(self.uttrs[idx]).removeprefix(str(self.record_dir) + '/')
        scene = save_to_file.split('/')[0]
        speakerId = save_to_file.split('/')[2]
        filename = save_to_file.split('/')[3]
        utteranceId = filename.split('_')[4]
        # Take 9 consecutive channel 0-8, 9-18 then 0 and 10 are the starting channel
        starting_channelId = filename.split('_')[-1].split('.')[0] # Get the last character of the last segment
        paras = {
            'index': index,
            'seed': seed,
            ...
            'speakerId': speakerId,
            'starting_channelId': starting_channelId,
        }
